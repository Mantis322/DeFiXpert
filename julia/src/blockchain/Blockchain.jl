...
Provides a unified interface for interacDEFAULT_RPC_URLS = Dict(
    "ethereum" => "https://mainnet.infura.io/v3/YOUR_INFURA_KEY",
    "polygon"  => "https://polygon-rpc.com",
    "solana"   => "https:/function get_balance_generic(address::String, connection::Dict)::Float64
    ...
    try
        if network == "solana"
            ...
            return balance_lamports / 1e9 # Convert lamports to SOL (1 SOL = 1e9 lamports)
        elseif network == "algorand"
            # Use AlgorandClient for balance retrieval
            config = AlgorandClient.AlgorandConfig(node_url=endpoint)
            provider = AlgorandClient.create_algorand_provider(config)
            return AlgorandClient.get_balance_algo(provider, address)
        else
            # Assumes EVM chain
            hex_balance = _make_generic_rpc_request(endpoint, "eth_getBalance", [address, "latest"])
            balance_wei = parse(BigInt, hex_balance, base=16)
            return Float64(balance_wei) / 1e18 # Convert wei to ETH
        end
    catch e
        @error "get_balance_generic: Error getting balance for address $address on $network" error=e
        return 0.0
    end
endlana.com",
    "algorand" => "https://mainnet-api.algonode.cloud"
    # Add other common defaults if desired
)ith various blockchain networks,
...
include("AlgorandClient.jl") # Algorand blockchain support
...
using .AlgorandClient  # Algorand blockchain support
...
# Algorand-specific exports
export create_algorand_provider, get_balance_algo, get_account_info_algo, get_asset_info_algo
export send_transaction_algo, create_payment_transaction_algo, create_asset_transfer_transaction_algo 
...
function _load_blockchain_config()
    ...
    for chain_key_any in supported_chains_list
        ...
        chain_id = if chain_key == "ethereum"; 1
                     ...
                     elseif chain_key == "algorand"; -2 # Special value for Algorand
...
function is_node_healthy_generic(network_name::String, endpoint_url::String)::Bool
    try
        if network_name == "solana"
            ...
            return result == "ok"
        elseif network_name == "algorand"
            # Algorand-specific health check
            headers = Dict("Content-Type" => "application/json")
            response = HTTP.get(endpoint_url * "/v2/status", headers)
            result = JSON3.read(response.body)
            return haskey(result, "last-round") && result["last-round"] > 0
...
function get_chain_id_generic(connection::Dict)::Int
    ...
    if connection["network"] == "solana"
        ...
        return -1 
    elseif connection["network"] == "algorand"
        # Algorand uses genesis-id instead of numeric chain ID
        @warn "get_chain_id_generic: Algorand does not use numeric chain IDs like EVM chains. Returning conventional -2."
        return -2
    end
    # Assumes EVM chain
